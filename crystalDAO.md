## crystalDAO
不是这题真有点sb，我看了半天没找到问题在哪，最后看见一个题解的注释恍然大悟，进行克隆部署最小代理合约，最小代理合约调用实现合约是delegatecall的，即使之前owner被初始化了，但是owner只在实现合约上下文有效，最小合约调用时owner是最小代理合约的owner，再次进行初始化的时候owner没有被初始化，也就是owner没有被定义过，所以owner是0地址，erecover报错也会返回0地址，就过了。
